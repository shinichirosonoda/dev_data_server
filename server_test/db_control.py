...
def data_create(board_id, file_path, flag, date):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    date = datetime.datetime.now()
    text = 'insert into board(board_id, file_path, flag, date) values(?,?,?,?)'
    c.execute(text, (board_id, file_path, flag, str(date)))
    data = c.lastrowid
    conn.commit()
    conn.close()
...
def update_data(board_id, file_path, flag, date):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    text = 'update board set file_path = ? where board_id = ?'
    c.execute(text,(file_path, board_id))
