...
from db_control import data_create_board, data_create_fov, data_create_single,\
...
@app.route('/data_create_single', methods=['POST'])
def f_data_create_single():
    # jsonから変換
    data_json = request.data.decode('utf-8')
    data_dict = json.loads(data_json)

    # DBにレコードを作成
    id = data_create_single(data_dict)
